...
public sealed class ImpinjReaderConnection : IReaderConnection
{
    ...
    public ImpinjReaderConnection(ReaderConfig config)
    {
    ...
    public Task ConnectAsync(CancellationToken ct = default)
    {
        ...
        UpdateState(ConnectionState.Connecting, "Connecting to Impinj Reader...");
        try
        {
            string host = _config.IpAddress ?? "192.168.1.100";
            UpdateState(ConnectionState.Connected, $"Connected to Impinj Octane/IoT at {host}");
            return Task.CompletedTask;
        }
        catch (Exception ex)
        {
            UpdateState(ConnectionState.Faulted, $"Impinj connection failed: {ex.Message}", ex);
            throw;
        }
    }
    ...
    public Task DisconnectAsync(CancellationToken ct = default)
    {
        ...
        StatusChanged?.Invoke(this, new ReaderStatusEventArgs(ReaderId, oldState, ConnectionState.Disconnected, "Impinj Disconnected"));
        return Task.CompletedTask;
    }
    ...
    public void ProcessTagReport(string epc, float rssi, int antennaPort, string? tid = null)
    {
